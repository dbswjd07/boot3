...
public interface BoardRepository extends CrudRepository<BoardVO, Integer> {
	...
	public List<BoardVO> findBoardVOByTitle(String title); //쿼리 메서드 중에서 find+엔티티빈
	//클래스명 + By + 테이블컬럼()
	...
	public Collection<BoardVO> findByTitleContainingOrContentContaining(String string, String string2);
	
	//title like %?% And Bno > ?
	public Collection<BoardVO> findByTitleContainingAndBnoGreaterThan(String title,
			int bno);
	
	//bno > ? order by bno besc => 게시물 번호가 특정번호보다 큰 게시물을 bno기준으로 내림차순 정렬
	...
	//클래스명을 이용하고, 실제 테이블의 컬럼명 대신 엔티티빈의 변수 즉 속성을 사용한다. ?1의 뜻은 첫번째로 전달되는
	//인자값을 의미한다. 
	...
	//원하는 컬럼만 검색할 때는 반환타입이 컬렉션 제네릭 타입이 엔티티빈 타입이 아니라 Object[] 배열이다.
